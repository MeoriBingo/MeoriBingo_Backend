...
# src.app.models.mission 모듈이 실제로 존재하는지 확인이 필요합니다.
# 만약 아래에 정의된 Mission 클래스를 사용한다면 이 import는 주석 처리해야 합니다.
...
class BingoAIService:
    ...
    def generate_bingo_missions(self, mode: str, selected_category: str) -> List[Dict]:
        ...
        {category_definitions} # 정의도 프롬프트에 포함시키는 것이 좋습니다.
        ...
        try:
            ...
            print(f"Generate Bingo Missions Error: {e}")
            return []
    ...
    def verify_image_mission(self, db: Session, mission_id: int, image_path: str):
        ...
        :param mission_id: Mission 테이블의 PK (id)
        :param image_path: 판독할 이미지의 로컬 경로
        """
        # 1. DB에서 미션 정보 조회 (변수명을 mission_id로 변경하여 명확성 확보)
        ...
        # 3. Azure OpenAI 설정 (클래스 초기화 시 수행됨, 중복 코드 제거)
        # client = AzureOpenAI(
        ...
        # 4. GPT-4o 멀티모달 모델에게 판독 요청
        try:
            response = self.client.chat.completions.create(
                model=self.deployment, # self.deployment 사용
                messages=[
                    {
                        "role": "system",
                        "content": (
                ...
                max_tokens=10, # 성공/실패만 답하면 되므로 토큰을 아낄 수 있습니다.
                temperature=0 # 판독의 일관성을 위해 0으로 설정
            )
    ...
    def request_openai(self, mission_obj, lines=0, streak=1, weather="맑음", completed_at=None, histories=[]):
        """
        사용자의 미션/빙고 달성 상황에 맞춰 축하 문구를 생성합니다.
        """
        # 1. 시간 및 요일 정보 처리
        ...
        # 안전한 샘플링: 풀의 크기가 2보다 작을 경우를 대비합니다.
        k_size = min(len(context_pool), 2)
        selected_keys = random.sample(list(context_pool.keys()), k_size)
...
"""
        ...
        # histories가 유효한 형식의 리스트인지 확인하는 것이 좋습니다.
        if isinstance(histories, list):
            messages.extend(histories)
        messages.append({"role": "user", "content": user_content})

        try:
            # [수정] "self.client"와 "self.deployment"를 사용합니다.
            response = self.client.chat.completions.create(
                model=self.deployment, # self.deployment 사용
                messages=messages,
                temperature=0.9,
            ...
            print(f"Request OpenAI (Greeting) Error: {e}")
            return "미션 완료를 축하합니다! 정말 멋져요." # 기본 축하 문구 반환