...
router = APIRouter(prefix="/api/admin/point", tags=["Admin Point"])
...
async def get_user_point_history(
    ...
    history = db.query(PointLog).filter(PointLog.user_id == user_id).order_by(PointLog.created_at.desc()).all()
